...
void lin_Init(void)
{
    ...
    GPIOC -> MODER |= 0x000000A0;               // set Pin 6, 7 to Alternate Mode//A000 for pin 6 and 7???
    GPIOC -> AFR[0] |= 0x76000000;              // select alternate functions  //SPI3 for pin 6 and usart 1..3 for pin 7 
    RCC -> APB1ENR |= 0x00000020;               // enable clock for usart6, tim3
    USART6 -> BRR = 0x00001117;                 // set baudrate 273,4375  //why this??
    USART6 -> CR1 |= 0x0000206A;                // enable RE, TE, RXNEIE, TCIE, UE  //false enables 
